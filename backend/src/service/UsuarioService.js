...
const criarUsuario = async (userParam) => {
  try {
    ...
    let Usuario = await usuarioRepository.encontrarUsuarioPeloTelefone(
      telefone
    );
    if (Usuario) {
      return {
...
const login = async (telefone, senha) => {
  ...
  try {
    const senhaUsuario = await usuarioRepository.encontrarUsuarioPeloTelefone(
      telefone
    );
    ...
    if (senhaUsuario) {
      ...
      const usuario = await usuarioRepository.encontrarUsuarioPorSenhaEtelefone(
        telefone,
        hash
      );
      ...
      const refreshToken = jwt.sign({ usuario }, preocess.env.SECRET_KEY, {
        expiresIn: 8000,
      });
...
const refreshToken = async (token) => {
  ...
  const refreshToken = jwt.sign({ usuario }, process.env.SECRET_KEY, {
